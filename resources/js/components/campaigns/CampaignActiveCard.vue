...
<template>
  ...
  >
    <div class="flex items-start gap-4 border-b border-success-border/80 px-6 py-5">
      <div class="flex min-w-0 flex-1 items-start gap-4">
        ...
        <div class="min-w-0 flex-1">
          ...
          <h3 class="mt-2 line-clamp-2 text-xl font-black tracking-tight text-ink sm:text-2xl">
            {{ campaign.name }}
          </h3>
          <p v-if="campaign.summary" class="mt-1 line-clamp-2 text-sm text-ink-muted">
            {{ campaign.summary }}
          </p>
        </div>
      </div>
      <div
        class="grid size-[4.5rem] shrink-0 place-items-center rounded-2xl bg-primary text-center text-white shadow-lg sm:size-20"
      >
        <div>
          <p class="text-2xl font-black leading-none sm:text-3xl">
            {{ campaign.multiplier ?? campaign.config.stamp_multiplier ?? 2 }}×
          </p>
          <p class="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-ink-soft sm:text-xs">stamps</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-4 px-6 py-5">
      ...
      <p v-else-if="campaign.schedule_summary" class="min-h-[1.75rem] text-sm font-semibold text-ink-muted">

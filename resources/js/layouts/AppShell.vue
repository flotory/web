...
<template>
  <div class="min-h-screen md:grid md:grid-cols-[272px_1fr]">
    ...
    >
      ...
      <!-- min-h-0 lets this flex child actually scroll instead of overflowing
           into the Account/Logout footer when the viewport is short. -->
      <nav class="mt-6 min-h-0 flex-1 space-y-1 overflow-y-auto px-1">
        <RouterLink
          v-for="item in nav"
          :key="item.to"
          :to="item.to"

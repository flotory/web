...
/** Pure OAuth callback parsing — used by startGoogleBrowserSignIn and unit tests. */
export function googleAuthResultFromQueryParams(
  queryParams: Record<string, unknown> | null | undefined,
): GoogleBrowserAuthResult {
  const oauthToken = readQueryParam(queryParams?.oauth_token)
  if (oauthToken) {
...
export async function startGoogleBrowserSignIn(): Promise<GoogleBrowserAuthResult> {
  ...
  return googleAuthResultFromQueryParams(parsed.queryParams ?? undefined)
}
